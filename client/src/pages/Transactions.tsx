...
const Transactions = () => {
  ...
  const fetchTransactions = async () => {
    try {
      const res = await axios.get("${import.meta.env.VITE_API_URL}/api/transactions", { headers });
  ...
  const handleAdd = async () => {
    ...
    setLoading(true);
    try {
      await axios.post("${import.meta.env.VITE_API_URL}/api/transactions", form, { headers });
  ...
  const handleDelete = async (id: number) => {
    try {
      await axios.delete(`${import.meta.env.VITE_API_URL}/api/transactions/${id}`, { headers });
      fetchTransactions();
    } catch (err) {
      console.error(err);
    }
